...
// In-memory cache for call-to-lead mapping (CallSid -> lead data)
const callLeadCache = new Map();
...
async function initiateCall(leadPhone, leadName, leadAddress) {
  ...
  try {
    ...
    // conversation_initiation_client_data needs type and dynamic_variables per API docs
    const conversationData = {
      type: "conversation_initiation_client_data",
      dynamic_variables: {
        lead_name: leadName || 'Unknown',
        property_address: leadAddress || 'Unknown'
      }
    };
    ...
    // Store lead data in cache for webhook lookup
    storeLeadData(callId, {
...
/**
 * Get lead data from cache by CallSid
 * @param {string} callSid - The Twilio Call SID
 * @returns {Object|null} - Lead data or null
 */
function getLeadDataByCallSid(callSid) {
  return callLeadCache.get(callSid) || null;
}

/**
 * Store lead data in cache by CallSid
 * @param {string} callSid - The Twilio Call SID
 * @param {Object} leadData - Lead information
 */
function storeLeadData(callSid, leadData) {
  callLeadCache.set(callSid, leadData);
  // Auto-expire after 24 hours
  setTimeout(() => callLeadCache.delete(callSid), 24 * 60 * 60 * 1000);
}
