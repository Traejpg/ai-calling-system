...
class WebhookHandlers {
  ...
  async handleStatusWebhook(req, res) {
    ...
    try {
      switch (CallStatus) {
        case 'completed':
          await this.handleCompleted(CallSid, CallDuration, To, LeadId, RecordingUrl);
          break;
          
        case 'no-answer':
          await this.handleNoAnswer(CallSid, To, LeadId, From);
          break;
          
        case 'busy':
          await this.handleBusy(CallSid, To, LeadId, From);
          break;
          
        case 'failed':
          await this.handleFailed(CallSid, To, LeadId, req.body);
          break;
          
        case 'canceled':
          await this.handleCanceled(CallSid, To, LeadId);
          break;
  ...
  async handleCompleted(callSid, duration, to, leadId, recordingUrl) {
    logger.info('Processing completed call', { callSid, leadId });

    try {
      // Look up lead data from cache or by phone number
      let leadData = getLeadDataByCallSid(callSid);
      ...
      const leadName = leadData ? (leadData.name || leadData.properties?.Name?.title?.[0]?.plain_text || 'Unknown') : 'Unknown';
