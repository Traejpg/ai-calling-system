...
class WebhookHandlers {
  ...
  async handleStatusWebhook(req, res) {
    ...
    try {
      ...
      if (isVoicemail) {
        console.log('📼 Voicemail detected - handling...');
        await this.handleVoicemailDetected(conversationId, to, leadId, from, body);
        return;
      }

      console.log('📊 Status check - proceeding to handler for:', callStatus);
      ...
      try {
        await this.sendOutcomeSMS('Unknown', to || 'unknown', 'WEBHOOK ERROR', error.message);
      } catch (smsError) {
  ...
  async handleVoicemailDetected(callSid, to, leadId, from, body) {
    ...
    try {
      ...
      // Send SMS alert for voicemail
      await this.sendOutcomeSMS(leadName, to, 'VOICEMAIL LEFT', '30-sec message dropped');
...
<Response>
  ...
  /**
   * Send SMS alert for call outcome
   */
  async sendOutcomeSMS(leadName, phone, outcome, details = '') {
    try {
      const smsAlertNumber = process.env.SMS_ALERT_NUMBER || process.env.AGENT_PHONE_NUMBER;
      if (!smsAlertNumber) return;
      
      const twilio = require('twilio');
      const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
      
      const message = `📞 CALL OUTCOME\n\nLead: ${leadName}\nPhone: ${phone}\nResult: ${outcome}\n${details}`;
      ...
      logger.info('SMS alert sent', { leadName, outcome });
    } catch (error) {
  ...
  async handleCompleted(callSid, duration, to, leadId, recordingUrl) {
    ...
    try {
      ...
      // Send SMS alert
      await this.sendOutcomeSMS(leadName, to, 'COMPLETED', `Duration: ${duration}s`);
  ...
  async handleNoAnswer(callSid, to, leadId, from) {
    ...
    // Get lead name for SMS
    let leadData = await this.notionService.findLeadByPhone(to);
    const leadName = leadData ? (leadData.name || 'Unknown') : 'Unknown';
    
    // Send SMS alert
    await this.sendOutcomeSMS(leadName, to, 'NO ANSWER', 'Will retry in 15 min');
  ...
  async handleBusy(callSid, to, leadId, from) {
    ...
    // Send SMS alert
    await this.sendOutcomeSMS(leadName, to, 'BUSY', 'Will retry in 15 min');
  ...
  async handleFailed(callSid, to, leadId, body) {
    ...
    // Send SMS alert
    await this.sendOutcomeSMS(leadName, to, 'CALL FAILED', `${failureType}: ${errorMessage}`);
