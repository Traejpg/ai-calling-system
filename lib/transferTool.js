...
async function transferToAgent(params) {
  ...
  try {
    ...
    // Get the actual call details to find the lead's number
    const callDetails = await client.calls(call_sid).fetch();
    const leadPhoneNumber = callDetails.to || lead_phone;
    
    console.log(`   Actual lead number: ${leadPhoneNumber}`);
    ...
    // Send SMS alert with correct lead number
    await sendTransferSMS(lead_name, property_address, leadPhoneNumber);
